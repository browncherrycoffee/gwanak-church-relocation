...
const RATIONALE: { no: number; title: string; detail: string; urgency: Urgency }[] = [
  ...
  },
  {
    ...
    title: "주차 갈등",
    detail:
      "건물 주차 불가로 별도 주차비 지출(월 약 70만원 고정) + 건물주·방문자와 마찰",
    urgency: "low",
  },
];
...
const COST_COMPARE: {
  ...
  { item: "주차비", current: "약 70만원 (고정)", after: "약 21만원 (정기 4대 12 + 할인권 9)", delta: "-약 50만원", deltaTone: "down" },
  { item: "관리비·공용·전기·수도", current: "40~200만원", after: "약 167만원 (실측 평균)", delta: "실측 반영", deltaTone: "neutral" },
  { item: "합계", current: "340~500만원", after: "약 540~575만원", delta: "+약 140~175만원", deltaTone: "up" },
];
...
const SAVING_POINTS: { item: string; ratio: string; note: string }[] = [
  ...
  { item: "주차비 절감", ratio: "—", note: "이전 자체로 월 약 58만원 절감 (70만 → 12만, 가장 확실한 절감 항목)" },
...
const BOKBOK_FEE: {
  ...
  total: "약 188만원 / 월",
  note: "목사님·안장로 방문 시 안내받은 ‘관리비 140만원’은 세대분(세대관리비 91 + 세대전기 48). 여기에 공용관리비(약 28)·주차비(약 21)가 더해져 월 총 부과액은 평균 약 188만원. 주차비는 현 예배당(약 70만)보다 크게 낮지만, 세대관리비(약 91만)는 매입(소유) 시 새로 생기는 비용.",
};
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        kind: "확인 완료",
        check: "16개월 부과 명세 확보 — 월 평균 약 188만원(세대관리 91+세대전기 48+공용 28+주차 21). 주차는 현 예배당(약 70만)보다 크게 낮으나, 세대관리비(약 91만)는 매입 시 새로 생기는 비용. 비교표·Plan A에 실측 반영.",
      },
      {
...
const CONCLUSIONS: {
  ...
  },
  {
    ...
    conclusion:
      "본질은 ‘부동산 가치 상승’이 아니라 ‘영속적으로 안정된 예배 공간의 확보’입니다. 가치 상승은 부차적이며, 시세가 그대로여도 매달 사라지던 임대료가 ‘우리 공간’으로 남는 것만으로 의미가 있습니다. ‘투자 자산’이 아니라 ‘신앙의 터전’으로 접근하되, 만일에 대비해 매입가 적정성과 대출 규모는 보수적으로 검증합니다.",
  },
  {
    no: 3,
    issue: "재정 감당 가능성",
      ...
      <Card>
        ...
        <CardContent className="flex flex-col gap-4">
          ...
          <Note>
            ...
            또한 ‘관악구 새 임대’를 주장하는 의견에 대해서는, 숫자와 무관하게 성립하는 세
            가지를 함께 짚어야 합니다 — ① <strong>현 임대의 구조적 불안정(재계약·인상·퇴거)이
            그대로 반복</strong>되고 ② 예배당 용도의 <strong>인테리어·시설비와 종교시설 용도
            허가를 새로 부담</strong>해야 하며 ③ 엘리베이터·주차를 갖춘 대형 공간은 관악구에서도
            드물고 비쌉니다. 다만 매매의 리스크(원금 상환 부담·집합건물 환금성)는 실재하므로,
            <strong> 관악구 임대 실시세를 조사해 수치를 확정한 뒤 최종 비교</strong>하는 것을
            권합니다.
          </Note>
        </CardContent>
      </Card>
