...
export const PASTORAL_NOTES_SEED: PastoralNote[] = [
  {
    ...
    content: `예배당 이전과 관련하여 좀 간곡한 말씀을 올립니다.

최근 서울 소재 고신교회 중 상가임대 상황에서 어려움을 겪는 교회들이 있습니다. 오랜 기간 관계를 맺었던 임대인의 세대교체 — 곧 사망 후 상속세 납부를 위해 건물 자체가 매각되거나, 임대료 상승이 가파른 경우들입니다.

문제는 이런 교회들이 교회 존립을 위협받을 정도로 대체 장소를 구하기가 쉽지 않다는 것입니다.

우리 교회가 사용하는 장소도 앞으로 여러 변화에 내몰릴 수 있습니다. 현재 임대인이 언제까지 생존할지도 알 수 없고, 서울 지역은 아파트 한 채 가격만 해도 10억을 쉽게 부르는 도시입니다. 이런 곳에서 장소를 확보하는 것은 교회 존립과도 직결된 문제입니다.

지금 지불하는 월세도 10년을 계산하면 5억에 가깝습니다. 매입을 해서 이자와 원금을 갚아가면 장소가 남지만, 월세는 없어지는 돈입니다.
